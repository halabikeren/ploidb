...
def get_itis_db(db_link: str, db_dir: str):
    ...
    db_prefix = \
    [path for path in os.listdir(db_dir) if path.startswith("itis") and os.path.isdir(f"{db_dir}/{path}")][0]
    db_path = f"{db_dir}/{db_prefix}/ITIS.sqlite"
    return db_path

def get_plant_kingdom_id(db_connection: sqlite3.Connection) -> int:
    return pd.read_sql_query("SELECT kingdom_id from kingdoms WHERE kingdom_name LIKE 'Plant%' --case-insensitive;",
                             db_connection).values[0][0]
...
def get_rank_name(item: pd.Series, rank_name: str, ranks_data: pd.DataFrame, taxonomic_data: pd.DataFrame) -> str:
    ...
    while tax_order_diff > 0:  # go up by one rank
        ...
        tax_order_diff = ordered_ranks.index(rank_name) - ordered_ranks.index(
            item.rank_name)  # re-compute the taxonomic diff
    if tax_order_diff < 0:
        return np.nan
...
def get_ncbi_tax_id(tax_name: str) -> int:
    ...
    try:
        ...
        logger.warning(f"could not retrieve ncbi tax id fro {tax_name} due to error {e}")
    return tax_id

def fill_missing_data_from_itis(input_df: pd.DataFrame, input_col: str, db_link: str, db_dir: str):
    ...
    taxonomic_data["family_name"] = taxonomic_data.apply(
        lambda row: get_rank_name(item=row, rank_name="family", ranks_data=ranks_data,
                                  taxonomic_data=taxonomic_data),
        axis=1)
    ...
    for col in ["taxon_rank", "genus", "family"]:
        if col not in input_df.columns:
            input_df[col] = np.nan
    input_df["taxon_rank"].fillna(value=taxonomic_data.set_index("complete_name")["rank_name"].to_dict(),
                                  inplace=True)
    input_df["genus"].fillna(value=taxonomic_data.set_index("complete_name")["genus_name"].to_dict(), inplace=True)
    input_df["family"].fillna(value=taxonomic_data.set_index("complete_name")["family_name"].to_dict(),
                              inplace=True)
    return input_df
...
def get_tax_id_to_tax_data(tax_ids: list[int]) -> tuple[dict[int, str], dict[int, str]]:
    ...
    try:
        ...
        logger.warning(f"could not fetch tax data for {len(tax_ids)} tax ids due to error {e}")
